...
def getActorMovieIds(actorId):
	...
	return myDbCurr.fetchall()
def getMovGenre(movId):
	myDbCurr.execute("select genre from mlmovies where movie_id = ?", [movId])
	return myDbCurr.fetchall()
...
def getUserMovies(userId):
	myDbCurr.execute("select movie_id from mlratings where user_id = ?", [userId])
	return myDbCurr.fetchall()
