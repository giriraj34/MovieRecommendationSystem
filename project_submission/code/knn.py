...
def loadDataset(filename, trainingSet=[] , testSet=[]):
	...
	#dataset_copy = np.zeros((len(movies),allTagLen+1))
...
def knn():
	...
	csvArr = [['' for i in range(3)] for j in range(len(test)+1)]
	csvArr[0][0] = 'movie_id'
	csvArr[0][1] = 'movie_name'
	csvArr[0][2] = 'class'
	for i in range(len(test)):
		neighbors = getNeighbors(trainingSet, testSet[i], k,trainlist)
		result = getResponse(neighbors)
		predictions.append(result)
		mvName = di.getMovName(test[i])
		print("for movie :",test[i],mvName[0]," > predicted=" ,repr(result))
		csvArr[i+1][0] = test[i]
	...
	#print(csvArr)
	np.savetxt('resKNN.csv', csvArr, fmt='%s',delimiter = ',') 
