...
class lshIdx:
    ...
    def query(self,q,metric,max_results):
        ...
        for g,table in self.hTables:
            ...
            movArr.extend(matches)
        print("total movies considered = ",len(movArr))
        print("unique movies considered = ",len(np.unique(movArr)))
...
class LSHTester:
    ...
    def run(self,name,metric,hFamliy,kVal,lVal,qMov):
        irmov = len(irrMovies)
        exact_hits = [[ix for ix,dist in self.linear(q,metric,self.num_neighbours+1+irmov)] for q in self.queries]
        ...
        for q in self.queries:
            lsh_hits = [(ix,dist) for ix,dist in lsh.query(q,metric,self.num_neighbours+1+irmov)]
        print("ser no.\tmovieIds\tmovieNames")
        i = 0
        retArr = []
        for l,d in lsh_hits:
            if(movies[l] != qMov):
                if(movies[l] not in irrMovies):
                    i += 1
